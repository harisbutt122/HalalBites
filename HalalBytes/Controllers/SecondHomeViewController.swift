...
//        
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let navigationController = self.navigationController as? ScrollingNavigationController {
            navigationController.followScrollView(CollectionVIew, delay: 0.0, followers: [NavigationBarFollower(view: SecondView , direction: .scrollUp)])
            navigationController.scrollingNavbarDelegate = self
        }
    }
    @IBAction func LoginFromBarButton(_ sender: Any) {
    ...
    @IBAction func AddRestaurants(_ sender: Any) {
        let alertVC = PMAlertController(title: "", description: "For add restaurants, you need to login first!!", image:UIImage(named: "halal"), style: .alert)
...
extension SecondHomeViewController:UICollectionViewDelegate,UICollectionViewDataSource, UISearchResultsUpdating, UIViewControllerTransitioningDelegate,UITableViewDelegate,UITableViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        searchController.searchBar.resignFirstResponder()
        self.performSegue(withIdentifier: "Details1", sender: self)
    }
